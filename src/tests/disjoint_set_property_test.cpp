...
static souffle::PiggyList<parent_t> randomize_disjoint_set(
    ...
    auto random_node = [&nodes] { return nodes.get(random() % nodes.size()); };
    // Must be non-empty
    ...
    for (std::size_t i = 0; i < (random() % max_size) + min_size; ++i) {
...
#pragma omp parallel for
#endif
    for (std::size_t i = 0; i < RANDOM_TEST_SIZE; ++i) {
...
#pragma omp parallel for
        ...
        for (int j = 0; j < RANDOM_TEST_SIZE; ++j) {
            auto size0 = ds.size();
            ds.findNode(nodes.get(random() % nodes.size()));
...
#pragma omp parallel for
        ...
        for (int j = 0; j < RANDOM_TEST_SIZE; ++j) {
            auto size0 = ds.size();
            ds.unionNodes(nodes.get(random() % nodes.size()), nodes.get(random() % nodes.size()));
            EXPECT_EQ(size0, ds.size());
        }
    }
}
...
#endif
        for (int j = 0; j < RANDOM_TEST_SIZE; ++j) {
            parent_t node1 = nodes.get(random() % nodes.size());
            parent_t node2 = nodes.get(random() % nodes.size());
...
#endif
        for (int j = 0; j < RANDOM_TEST_SIZE; ++j) {
            parent_t node = nodes.get(random() % nodes.size());
...
TEST(DjTest, UnionRoot) {
    for (int i = 0; i < RANDOM_TESTS; ++i) {
        ...
        for (int i = 0; i < random() % RANDOM_TEST_SIZE; ++i) {
            souffle::block_t x = ds.get(nodes.get(random() % nodes.size()));
            souffle::block_t y = ds.get(nodes.get(random() % nodes.size()));
